...
pub enum Node {
  ...
  Comparison {name: Vec<u8>, children: Vec<Node> /*two identifiers*/}, //new
...
pub fn t_less(input: Tokens) -> IResult<Tokens, Token> {
  let fxn = check_token(& |tk| match tk.kind {
...
pub fn comparison(input: Tokens) -> IResult<Tokens, Node> {
  let (input, leftside) = value(input)?;
  let (input, operator) = alt((t_greater,t_less,t_greater_equal,t_less_equal,t_equal_to,t_not_equal))(input)?;
  let (input, rightside) = value(input)?;
  let name = match operator.kind {
    TokenKind::GreaterThan => b">".to_vec(),
    TokenKind::LessThan => b"<".to_vec(),
    TokenKind::GreaterEqual => b">=".to_vec(),
    TokenKind::LessEqual => b"<=".to_vec(),
    TokenKind::EqualTo => b"==".to_vec(),
    TokenKind::NotEqualTo => b"!=".to_vec(),
    _ => unreachable!(),
  };
  Ok((input, Node::Comparison{name: name, children: vec![leftside, rightside] }))
}
