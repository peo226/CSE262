...
pub fn lex(input: &str) -> Tokens {
  ...
  while i < list.len() {
    ...
    if i + 1 < list.len()  && list[i+1] == 61 {
      match kind{
        TokenKind::GreaterThan => {kind = TokenKind::GreaterEqual; i+= 1;},
        TokenKind::LessThan => {kind = TokenKind::LessEqual; i+= 1;},
        TokenKind::Equal => {kind = TokenKind::EqualTo; i+= 1;},
        TokenKind::Exclamation => {kind = TokenKind::NotEqualTo; i+= 1;},
        _ => {}
      }
    }
    //create token struct
